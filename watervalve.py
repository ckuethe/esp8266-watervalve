...
response_time = 5  # detection latency, in seconds
flow_poll_ms = 500  # how often to poll water flow state


flow_num_samples = response_time * 1000 // flow_poll_ms
flow_samples = []
valve_state = None
...
def sample_flow(tmr=None):
    ...
    if tmr:
        tmr.init(period=flow_poll_ms, mode=Timer.ONE_SHOT, callback=sample_flow)
...
def main():
    ...
    flow_timer = Timer(-1)
    flow_timer.init(period=flow_poll_ms, mode=Timer.ONE_SHOT, callback=sample_flow)
